...
// Postcondtion: A new populated Question exists
template <class T>
Question<T>::Question(const Question& Q){
  Q.m_subject = m_subject;
  Q.m_question = m_question;
  Q.m_datatype = m_datatype;
  Q.m_difficulty = m_difficulty;
  Q.m_answer = m_answer;
}
...
ostream& operator<< (ostream& output, Question<Y>& Q){
  output << "Question: " << Q.m_question << endl <<
